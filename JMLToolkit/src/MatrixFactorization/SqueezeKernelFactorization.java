...
public class SqueezeKernelFactorization 
{
	...
	// constructor
	public SqueezeKernelFactorization(DataSet trainSet, DataSet testSet, 
			int dimensions, double boxConstraing, double polynomialDegree, 
	...
	public void SolveUV()
	{
		...
		while(!converged)
		{
			...
			for(int i = 0; i < n_train; i++)
			{
				...
				{
					// get dot product of U_i* and U_l*
					double dp = DotProductU(i, l);
					
					double tmp = (1-beta)*coeff.get(i)*coeff.get(l)
									*p*Math.pow(dp + 1, p-1);
	...
	// the dot product of two vectors of the latent space 
	public double DotProductU(int i, int l)
	{
		// the dot product
		double dp = 0;
		for(int k = 0; k < d; k++)
			dp += U.get(i, k)*U.get(l, k);
	...
	public double GetFCALoss()
	{
		...
		for(int i = 0; i < n_train; i++)
			for(int l = 0; l < n_train; l++)
				sum += coeff.get(i)*coeff.get(l)*Math.pow( DotProductU(i, l) + 1 , p);
	...
	public double GetMCRTrainSet() 
	{
		...
		for(int i = 0; i < n_train; i++)
		{
			double sum = 0;
			for(int l = 0; l < n_train; l++)
				sum += coeff.get(l)*Math.pow(DotProductU(i, l) + 1, p);
			sum -= svmModel.rho[0];
			...
			// check for overflow, count it as error
			if(sum == Double.NaN)
				errors++;
		}
